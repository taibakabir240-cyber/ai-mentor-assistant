    ...
    with nav_tabs[0]:
        ...
        with col1:
            ...
            if st.button(clear_lbl):
                st.session_state.student_messages = [{"role": "assistant", "content": "چیٹ ہسٹری صاف कर दी गई है।" if is_urdu else "Chat history cleared."}]
                st.session_state.last_audio_signature = None
                st.rerun()
        with col2:
        ...
        for message in st.session_state.student_messages:
            ...
            with st.chat_message(message["role"], avatar=avatar):
                ...
                if message["role"] == "assistant":
                    try:
                        # Fixed: Dynamically switch TTS language based on current setting ('ur' for Urdu, 'en' for English)
                        tts_lang = 'ur' if is_urdu else 'en'
        ...
        def submit_whatsapp_query():
            ...
            if q:
                ...
                if not client:
                    ...
                    try:
                        if is_urdu:
                            sys_prompt = "آپ ایزی ٹیک فریم ورک کے لیے طیفہ کے پروفیشنل اے آئی مینٹور ہیں۔ براہ کرم اردو زبان میں واضح اور مددگار جواب دیں۔"
                        else:
                            sys_prompt = "You are Taiba's professional AI Mentor for Ezitech Engineering Framework (AI-003). Give concise, direct answers and helpful guidance."
            ...
            if client and len(audio_bytes) > 0 and audio_sig != st.session_state.last_audio_signature:
                ...
                with st.spinner(spin_txt):
                    try:
                        ...
                        # Fixed: Whisper language detection support (uses 'ur' if Urdu is selected, otherwise auto/en)
                        transcription_lang = "ur" if is_urdu else "en"
    ...
    with mentor_tabs[4]:
        ...
        if st.button("Broadcast Now"):
            if announcement:
                ...
                st.warning("Please enter a message to broadcast.")import os
import sqlite3
import streamlit as st
import pandas as pd
from datetime import datetime
    ...
    with mentor_tabs[4]:
        ...
        if st.button("Broadcast Now"):
            if announcement:
                ...
                st.warning("Please enter a message to broadcast.")v
